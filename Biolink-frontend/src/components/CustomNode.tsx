...
import { Activity, AlertTriangle, ShieldAlert, Flame, CheckCircle2 } from 'lucide-react';
...
export default function CustomNode({ data }: { data: any }) {
  ...
  // State Props
  const { isHeatmapMode, heatScore = 0, isTreated, isPrevented } = data;

  let nodeStyles = `relative w-[300px] rounded-2xl shadow-sm border-2 p-5 flex flex-col gap-2 transition-all duration-500 bg-white cursor-pointer hover:-translate-y-1 hover:shadow-xl `;
  
  // 🔥 Treatment Mode Overrides (Highest Priority)
  if (isTreated) {
    nodeStyles += 'border-emerald-500 bg-emerald-50 shadow-emerald-200 ring-4 ring-emerald-500/30';
  } else if (isPrevented) {
    nodeStyles += 'border-slate-200 opacity-30 grayscale scale-95';
  } 
  // 🔥 Heatmap Mode
    ...
    <div className={nodeStyles}>
      ...
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2">
          {/* Change icon to a checkmark if treated */}
          {isTreated ? <CheckCircle2 className="w-4 h-4 text-emerald-600" /> : (
            <>
          ...
          <div className="flex flex-col">
            <span className={`font-bold text-sm leading-tight ${isPrevented ? 'line-through text-slate-400' : 'text-slate-800'}`}>
              {data.label}
            </span>
            <span className={`text-[10px] font-extrabold uppercase tracking-wider 
              ${isTreated ? 'text-emerald-600' : isPrevented ? 'text-slate-400' : isDeficiency ? 'text-amber-600' : isDisease ? 'text-rose-600' : 'text-blue-600'}`}
            >
              {isTreated ? 'TREATED' : isPrevented ? 'PREVENTED' : data.type}
            </span>
          </div>
        </div>

        {/* Heat Score Badge */}
        {isHeatmapMode && heatScore > 0 && !isPrevented && !isTreated && (
      ...
      <p className={`text-xs line-clamp-4 font-medium leading-relaxed ${isPrevented ? 'text-slate-300' : 'text-slate-500'}`}>
        {data.description}
      </p>
